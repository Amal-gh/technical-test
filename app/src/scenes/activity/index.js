...
const Activities = ({ date, user, project }) => {
  ...
  useEffect(() => {
    (async () => {
      // Transmettre le projectId au lieu du projet pour pouvoir faire la requête back sur le projectId
      const { data } = await api.get(`/activity?date=${date.getTime()}&user=${user.name}&project=${project?._id}`);
      ...
      <div className="w-screen md:w-full p-2 md:!px-8">
        ...
        {(
          <div className="mt-2 rounded-xl bg-[#fff] overflow-auto">
              ...
              <table className="w-full">
                  ...
                  return (
                    <React.Fragment key={e.projectId}>
                      <tr className="border-t border-b border-r border-[#E5EAEF]" key={`1-${e._id}`}
                          onClick={() => setOpen(i)}>
                        <th
                          className="w-[100px] border-t border-b border-r text-[12px] font-bold text-[#212325] text-left">
                          <div className="flex flex-1 items-center justify-between gap-1 px-2">
                            ...
                            <div className="flex flex-col items-end">
                              ...
                              <div
                                className="text-[10px] italic font-normal">{(((e.total / 8).toFixed(2) / getTotal()) * 100).toFixed(2)}%
                              </div>
                            </div>
                          </div>
                        </th>
                        {e.detail.map((f, j) => {
                          return (
                            <Field key={`${e.project} ${j}`} invoiced={e.invoiced} value={f.value || 0}
                      ...
                      {open === i && (
                        <tr className="border border-[#E5EAEF]" key={`2-${e._id}`}>
                          <th
                            className="w-[100px] border border-[#E5EAEF]  text-[12px] font-bold text-[#212325] text-left pl-[10px]">
                            <div></div>
                          </th>
                          <th colSpan="30">
                ...
                <tr>
                  <th className="w-[50px] text-[12px] text-[#212325] px-[10px] py-2">
                    <SelectProject disabled={activities.map((e) => e.project)} value=""
            ...
            <button
              className="m-3 w-[82px] h-[48px] py-[12px] px-[22px] bg-[#0560FD] text-[16px] font-medium text-[#fff] rounded-[10px]"
              onClick={onSave}>
              Save
            </button>
          </div>
